...
class EncodecModel(nn.Module):
    ...
    def decode2emb(self, encoded_frames: tp.List[EncodedFrame]) -> torch.Tensor:
        #Decode indices into embeddings
        segment_length = self.segment_length
        if segment_length is None:
            assert len(encoded_frames) == 1
            return self._decode_frame_emb(encoded_frames[0])

        frames = [self._decode_frame_emb(frame) for frame in encoded_frames]
    ...
    def _decode_frame_emb(self, encoded_frame: EncodedFrame) -> torch.Tensor:
        codes, scale = encoded_frame
        codes = codes.transpose(0, 1)
        emb = self.quantizer.decode(codes)
